...
EXEC sp_rename 'Products.ProductName', 'Product_Names', 'COLUMN';

-- EXEC sp_rename 'Products.Product_Names', 'ProductName', 'COLUMN';
...
SELECT DISTINCT FirstName, Country FROM Client

-- Write a query that uses CASE to create a conditional column that displays 'High' if Price > 1000, and
                                                                             -- 'Low' if Price <= 1000 from Products table.
SELECT Product_Names, Price, CASE
             WHEN Price > 1000  THEN 'High'
			 WHEN Price <= 1000  THEN 'Low'  ELSE 'Average' END AS Price_rate
FROM Products
...
SELECT 
  Product_Names, Price, 
  IIF(Price > 1000, 'Expensive', 'Affordable') AS Priceee
FROM Products ;
...
---Use orders table. Result set should show customerid, quantity and discount percentage. 
 ...
 SELECT CustomerID, Quantity, CASE
		 ...
		 WHEN Quantity BETWEEN 2 AND 3 THEN 0.05 ELSE 0.07 END AS discount_percentage
 FROM Orders
